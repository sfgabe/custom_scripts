...
mqtt_channel = "dht11-garden/reading"
mq = paho.Client()
username = "username"
password = "password"
host = "home-assistant-pi.local"
...
class DHT11(object):
    ...
    example code:
    >>> pi = pigpio.pi()
    >>> sensor = DHT11(pi, 23) # 23 is the data GPIO pin connected to your sensor
...
def mqtt_publish():
    ...
    mq.username_pw_set(username=(username),password=(password))
    mq.connect(host)
    with open(file, 'r') as myfile:
        data = myfile.read()
#        print data
    mq.publish(mqtt_channel,data)

if __name__ == '__main__':
    pi = pigpio.pi()
    sensor = DHT11(pi, 23)
    for d in sensor:
        print('Temp={0:0.1f}*C  Humidity={1:0.1f}% at '.format((d['temperature']), (d['humidity'])) + str(datetime.datetime.no$
        log = open(file, "w")
